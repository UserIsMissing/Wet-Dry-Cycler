...
/**
 * @brief Parses JSON and updates LED GPIO state based on "led" key.
 *
 * @param buffer JSON string received from server.
 */
void handle_led_response(const char *buffer) {
...
void poll_server_task(void *pvParameters) {
    ...
    // Configure HTTP client for LED state polling
    esp_http_client_config_t config = {
    ...
    while (1) {
        ...
        if (err == ESP_OK) {
            ...
            // Pass response to LED handler
            handle_led_response(buffer);
        ...
        vTaskDelay(pdMS_TO_TICKS(10)); // Delay before next iteration
    }
}
