...
// Movement motor instance (example pins, replace with actual)
static DRV8825_t movementMotor = {
...
void setup()
{
  ...
  DRV8825_Set_Step_Mode(&testMotor, DRV8825_FULL_STEP);

  DRV8825_Init(&movementMotor);
  DRV8825_Set_Step_Mode(&movementMotor, DRV8825_FULL_STEP);

  int steps = 1000000000;
  int delay_us = DRV8825_DEFAULT_STEP_DELAY_US;

  Serial.println("Moving movement motor...");
  DRV8825_Enable(&movementMotor);
  DRV8825_Set_Direction(&movementMotor, DRV8825_FORWARD);
  for (int i = 0; i < steps; i++)
  {
    if (DRV8825_Check_Fault(&movementMotor))
    {
      Serial.printf("[DRV8825] Fault detected on movement motor at step %d\n", i);
      break;
    }
  ...
  Serial.println("Movement motor move complete.");

  Serial.println("Moving rehydration motor...");
  DRV8825_Enable(&testMotor);
  DRV8825_Set_Direction(&testMotor, DRV8825_BACKWARD);
  for (int i = 0; i < steps; i++)
  {
    if (DRV8825_Check_Fault(&testMotor))
    {
      Serial.printf("[DRV8825] Fault detected on rehydration motor at step %d\n", i);
      break;
    }
    DRV8825_Step(&testMotor);
    delayMicroseconds(delay_us);
  }
  DRV8825_Disable(&testMotor);
  Serial.println("Rehydration motor move complete.");

  Serial.println("Both motors move complete.");
